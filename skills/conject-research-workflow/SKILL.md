...
3. Spawn one `conject_spawn_researcher` call per independent task. Each researcher should use `conject-research-agent` and `conject-source-evidence`, and each researcher receives its own bounded web-search budget.
